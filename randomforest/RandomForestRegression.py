...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  3 09:51:38 2021

@author: madeleip
Random forest regressor. Using sklearn RandomForestRegressor 

Input: Array of dependent and predictors variables. Xarray. 
Output: R2, Weights of Var Importance 
"""
...
def RandomForestRegression(features,N_train, N_test,name):
...
def ApplyRandomForestRegression(features,N_train, N_test,name,rf):
    ...
    #rf = RandomForestRegressor(n_estimators = 50, random_state = rng,oob_score=True)
   # rf.fit(X_train, y_train)                #Fit the RF model using the training data 
    ...
    # fig, ax = plt.subplots()
    # ax.bar(range(0,lst3.size),
    # lst3)
    # plt.xticks(range(0,lst3.size), lst2, rotation=90)
    # ax.set_xlabel("Permutation Importances")
    # ax.set_title(name)
    # ax.set_xlim(0,lst3.size)
    # font = {'family' : 'sans',
    #     'weight' : 'bold',
    #     'size'   : 12}
